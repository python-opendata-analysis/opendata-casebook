...
### Q: なぜインデックス生成は手動実行なのですか？

A: 以下の理由から手動実行方式を採用しています：
- 生成内容を確認してからコミットできる
- 権限設定が不要でシンプル
- 誤った更新を防げる
...
- 新しい事例を追加したとき
- metadata.yaml を更新したとき
- 定期的なメンテナンス時

### Q: 自動更新にしたい場合は？

A: プロジェクトの状況に応じて、以下の方式を検討できます：
- **PR作成方式**: 自動でPRを作成し、レビュー後にマージ
- **PAT使用方式**: Personal Access Tokenを使って自動コミット

詳細は `.github/workflows/` ディレクトリ内のサンプルファイルを参照してください。